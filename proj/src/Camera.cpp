...
void Camera::drawOneCameraSigment(sf::RenderWindow& window, double distance, int sigmentNum, double sectorWidth){
    ...
    double brightess = 255 * (viewH/SCREEN_HEIGHT);
...
//Для 3D
void Camera::drawCameraOnMap(sf::RenderTarget& target) {
    float minimapScale = 1.0f;

    this->render(target, minimapScale);

    double rayLength = 100.0;

    Point2D rayEnd;
    rayEnd.setX(this->position.getX() + rayLength * cos(direction));
    rayEnd.setY(this->position.getY() + rayLength * sin(direction));

    sf::VertexArray ray(sf::Lines, 2);
    ray[0].position = sf::Vector2f(
        this->position.getX() * minimapScale,
        this->position.getY() * minimapScale
        );
    ray[0].color = sf::Color::Green;

    ray[1].position = sf::Vector2f(
        rayEnd.getX() * minimapScale,
        rayEnd.getY() * minimapScale
        );
    ray[1].color = sf::Color::Green;
...
void Camera::drawOneCameraSigment(sf::RenderTarget& target, double distance, int sigmentNum, double sectorWidth){
    double realHeight = Object2D::height;
    double viewH;
    viewH = (realHeight - distance*tan(atan(realHeight/distance)- (PI/120)));
    sf::RectangleShape sigment(sf::Vector2f(sectorWidth, viewH*2));
...
void Camera::drawCameraView(sf::RenderTarget& target) {
    Point2D currRayEnd;
    bool isCrossed=false;
    double currRayDir;
    double rayDistance;
    ...
    for(double i=leftExtRay;i<rightExtRay;i+=rayInterval,segmentNum++){
        ...
        if(isCrossed){
            drawOneCameraSigment(target, rayDistance, segmentNum, raySectorWidth);
        }
    }
}
